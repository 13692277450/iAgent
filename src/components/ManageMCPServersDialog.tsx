...
//   /* --- 访问控制用户条目 --- */
//   .access-item {
//     @apply flex items-center gap-3 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/[0.06] transition-all duration-200;
//   }
//   .access-item:hover {
//     @apply bg-white/[0.06] border-white/[0.12];
//   }
// `;
...
//   const remove = async (id: number) => {
//     if (!confirm("确定删除这个 MCP Server 吗？")) return;
//     await fetch(`/api/admin/mcp_servers/${id}`, { method: "DELETE" });
//     await load();
//     onChanged?.(); // 👈 通知外层刷新
//   };
...
//           <div className="relative z-10">
//             <AdminTable
//               columns={["Name", "Connection", "Auth", "Permission", "Status"]}
//               loading={loading}
//               emptyText="No MCP servers configured. Click NEW SERVER to add one."
//               rows={list.map((s, idx) => ({
//                 id: s.id!,
//                 cells: [
//                   <span
...
//                   <span
//                     key="a"
//                     className="inline-flex items-center gap-1.5 text-slate-400 text-xs"
//                   >
//                     {(() => {
//                       const AuthIcon = AUTH_ICONS[s.auth_type] || Lock;
//                       return <AuthIcon className="w-3 h-3" />;
...
//                 type="button"
//                 onClick={() => setEditing(null)}
//                 className="group flex items-center gap-2 text-lg font-medium text-green-400 hover:text-cyan-200 transition-colors"
//               >
//                 <div className="p-3.5 rounded-md bg-white/5 group-hover:bg-orange-500/10 transition-colors">
//                   <ArrowLeft className="w-2.5 h-2.5" />
//                 </div>
//                 BACK TO LIST
//               </button>
...
//                   {editing.id ? editing.name || "Edit Server" : "New Server"}
//                 </span>
//               </div>
//             </div>
...
//               <div className="p-5 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-4 relative overflow-hidden">
//                 <div className="absolute top-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-amber-500/30 to-transparent" />
//                 <h3 className="text-xs font-semibold text-amber-400/80 uppercase tracking-[0.2em] flex items-center gap-2">
...
//                 <div className="space-y-2">
//                   {accessList.length === 0 ? (
//                     <div className="flex items-center justify-center py-6 rounded-lg border border-dashed border-white/[0.06] bg-white/[0.01]">
...
//                       <div key={a.id} className="access-item">
//                         {/* 用户头像占位 */}
//                         <div className="w-7 h-7 rounded-full bg-gradient-to-br from-cyan-500/20 to-blue-500/20 border border-white/10 flex items-center justify-center flex-shrink-0">
//                           <User className="w-3.5 h-3.5 text-cyan-300" />
//                         </div>
//                         <div className="flex-1 min-w-0">
//                           <span className="font-mono text-sm text-slate-200 block truncate">
//                             {a.username}
//                           </span>
...
//                     )}
//                   >
//                     <ChevronRight className="w-3.5 h-3.5 mr-1" /> Grant
//                   </Button>
//                 </div>
//               </div>
//             )}
